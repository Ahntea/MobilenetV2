...
early_stopping = EarlyStopping(patience = 3, verbose = True, path="/data/efficientnet/train/resnet.pt")
...
torch.save((running_loss_history,validation_running_loss_history), "/data/efficientnet/train/loss_resnet.pt")
torch.save((running_correct_history, validation_running_correct_history),"/data/efficientnet/train/acc_resnet.pt")

times = time.time()-starttime
with open("resnet_time.txt", "a") as f:
  f.write(str(times))